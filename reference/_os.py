...
if hasattr(os, "copy_file_range"):
    ...
    def _copy_file_range(source_fd, target_fd):
        ...
        offset = 0
        while True:
            sent = os.copy_file_range(
                source_fd, target_fd, blocksize, offset_dst=offset
            )
            if sent == 0:
                break  # EOF
            offset += sent
...
def copy_info(info, target, follow_symlinks=True):
    ...
    copy_times_ns = (
        ...
        and hasattr(info, "_mod_time_ns")
        and (follow_symlinks or os.utime in os.supports_follow_symlinks)
    )
    if copy_times_ns:
    ...
    copy_xattrs = (
        hasattr(info, "_xattrs")
        and hasattr(os, "setxattr")
        and (follow_symlinks or os.setxattr in os.supports_follow_symlinks)
    )
    if copy_xattrs:
    ...
    copy_bsd_flags = (
        hasattr(info, "_bsd_flags")
        and hasattr(os, "chflags")
        and (follow_symlinks or os.chflags in os.supports_follow_symlinks)
    )
    if copy_bsd_flags:
