//! PurePosixPath - Pure path with POSIX semantics.
...
impl PartialEq for PurePosixPath {
    fn eq(&self, other: &Self) -> bool {
        self.inner == other.inner
    }
}

impl Eq for PurePosixPath {}

impl Hash for PurePosixPath {
...
impl PurePosixPath {
    ...
    fn new(args: &Bound<'_, PyTuple>) -> PyResult<Self> {
        let inner = PurePath::from_args_with_flavor(args, PathFlavor::Posix)?;
        Ok(Self { inner })
    }
    ...
    fn __repr__(&self) -> String {
        format!("PurePosixPath('{}')", self.inner.to_str())
    }
    ...
    fn __truediv__(&self, other: &Bound<'_, PyAny>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: PurePath {
                parsed: self.inner.parsed.join(
                    &ParsedPath::parse(&other_str, PathFlavor::Posix),
                    PathFlavor::Posix,
                ),
    ...
    fn __rtruediv__(&self, other: &Bound<'_, PyAny>) -> PyResult<Self> {
        let other_str: String = other.extract()?;
        let other_parsed = ParsedPath::parse(&other_str, PathFlavor::Posix);
        Ok(Self {
            inner: PurePath {
                parsed: other_parsed.join(&self.inner.parsed, PathFlavor::Posix),
                flavor: PathFlavor::Posix,
            },
        })
    }
